...
let videoPlatform = window.videoPlatform || {};
...
registerForm.addEventListener('submit', async function (e) {
    ...
    // Проверяем параметр return из URL
    ...
    try {
        ...
        const user = {
            ...
            avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(firstName || email.split('@')[0])}&background=2e7d32&color=fff`
        };
        
        localStorage.setItem('videoPlatformUser', JSON.stringify(user));
...
window.location.href = returnUrl + (returnUrl.includes('?') ? '&' : '?') + 'auth_success=true';
